...
SaveUdpFlowMonitorStats (std::string filename, std::string simulationParams, Ptr<FlowMonitor> monitor, FlowMonitorHelper& flowmonHelper, double duration)
{
  ...
  outFile2.open ("throughput", std::ofstream::out | std::ofstream::app);
  if (!outFile2.is_open ())
    {
      NS_LOG_ERROR ("Can't open file " << "throughput");
      return;
    }
  outFile2.setf (std::ios_base::fixed);
      ...
      if (i->second.rxPackets > 0)
        {
          // Measure the duration of the flow from receiver's perspective
          // Mb/s
                       outFile2<<(i->second.rxBytes * 8.0 / rxDuration) / 1e6<< " ";
